...
class MkdocsExportConfluence(BasePlugin[MkdocsExportConfluenceConfig]):
    ...
    def __init__(self):
        ...
        self.config = {}
        self.enabled = True
        self.confluence_renderer = MyConfluenceRenderer(use_xhtml=True, enable_relative_links=True)
    ...
    def on_config(self, config: mkdocs.config.Config):
        ...
        self.session.auth = (self.config["confluence_username"], self.config["confluence_password"])
        
        self.session_file.headers.update({
            "X-Atlassian-Token": "nocheck"
        })
        self.session_file.auth = (self.config["confluence_username"], self.config["confluence_password"])

    def __process_config(self, config: mkdocs.config.Config):
        if config.get("confluence_host") is None and os.getenv("CONFLUENCE_HOST") is None:
            self.logger.info("Confluence host is required, disable plugin")
            self.enabled = False
            return
        else:
            self.config["confluence_host"] = config.get("confluence_host") or os.getenv("CONFLUENCE_HOST")
            
        if config.get("confluence_space") is None and os.getenv("CONFLUENCE_SPACE") is None:
            self.logger.info("Confluence space is required, disable plugin")
            self.enabled = False
            return
        else:
            self.config["confluence_space"] = config.get("confluence_space") or os.getenv("CONFLUENCE_SPACE")
        
        if config.get("confluence_username") is None and os.getenv("CONFLUENCE_USERNAME") is None:
            self.logger.info("Confluence username is required, disable plugin")
            self.enabled = False
            return
        else:
            self.config["confluence_username"] = config.get("confluence_username") or os.getenv("CONFLUENCE_USERNAME")
            
        if config.get("confluence_password") is None and os.getenv("CONFLUENCE_PASSWORD") is None:
            self.logger.info("Confluence password is required, disable plugin")
            self.enabled = False
            return
        else:
            self.config["confluence_password"] = config.get("confluence_password") or os.getenv("CONFLUENCE_PASSWORD")
            ...
            if item.structure == page:
                ...
                for attachment in self.confluence_renderer.attachments:
                    ...
                    item.confluence_body = re.sub(
                        ...
                        r'ri:filename="' + hashlib.md5(attachment.encode('utf-8')).hexdigest() + '"', 
    ...
    def __process_attachements(self):
        for item, attachment in self.attachements:
            ...
            url = self.config["confluence_host"] + "rest/api/content/" + item.confluence_id + "/child/attachment"
            ...
            response = self.session_file.request("PUT", url, files={
                "file": (attachement_name, open(attachement_path, "rb"), content_type),
                "comment": "Attachment for " + item.confluence_name,
            })
            
            if response.status_code != 200:
                raise PluginError(f"Failed to upload attachment: {response} {response.text}")
                ...
                if item.structure.is_page:
                    ...
                    if parsed_url == parsed_link:
                        ...
                        link[0].confluence_body = re.sub(
                            ...
                            r'<ac:link><ri:page ri:content-title="' + item.confluence_name + '" /><ac:link-body>\\1</ac:link-body></ac:link>', 
    ...
    def __process_confluence_page_id(self): 
        for item in self.items:
            ...
            if result is not None:
                self.logger.debug(f"Found page ID for {item.confluence_name}: {result['id']}")
    ...
    def __find_confluence_page_id(self, item):
        ...
        url = self.config["confluence_host"] + "api/v2/pages?title=" + page_name + "&space-id=" + self.config["confluence_space_id"] + ""
        self.logger.debug(f"Sending request to url: {url}")
        response = self.session.get(url)
        json = response.json()
    ...
    def __create_confluence_page(self, item):
        ...
        data = {
            ...
            "body": {"storage": {"value": item.confluence_body, "representation": "storage"}},
        }
...
class MyConfluenceRenderer(ConfluenceRenderer):
    def __init__(self, *args, **kwargs):
        ...
        self.title = None
        self.enable_relative_links = False
